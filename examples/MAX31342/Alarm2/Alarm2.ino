...
MAX31342::reg_status_t g_stat;

// Pin Number that connects to MAX31342 interrupt pin
...
void setup() {
    ...
    Serial.begin(115200);
    Serial.println("---------------------");
    Serial.println("ALARM2 use case example:");
    ...
    rtc_ctime.tm_mon  = 11;  // 0-11
    rtc_ctime.tm_mday = 05;  // 1-31
    ...
    ret = rtc.configure_inta_clkin_pin(MAX31342::CONFIGURE_PIN_AS_INTA);
    if (ret) {
        Serial.println("Configure inta failed!");
    }
    
    ret = rtc.set_alarm(MAX31342::ALARM2, &rtc_ctime, MAX31342::ALARM_PERIOD_EVERYMINUTE);
    if (ret) {
        Serial.println("Set alarm failed!");
    }

    ret = rtc.irq_enable(MAX31342::INTR_ID_ALARM2);
    if (ret) {
